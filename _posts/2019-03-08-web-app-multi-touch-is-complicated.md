...
*Originally published on [the Intersection blog](https://ixn.intersection.com/web-app-multi-touch-is-complicated-16e1db2b29ba)*
...
That said, we ran into a pretty interesting issue as we were building our latest kiosk experience. As you might recall from my post, [Building Smart City Kiosks with Web App Practices](https://ixn.intersection.com/building-smart-city-kiosks-with-web-app-practices-b99ccb1134f4), we built a web app, which is then loaded in a Chromium-based player to deliver that interactive kiosk experience. Almost immediately, we ran into this question:
...
Some solutions, such as [this post on Stack Overflow](https://stackoverflow.com/questions/37379694/javascript-how-to-stop-pinch-zoom-multi-touch-input-attacks), are a bit out of date. While a lot of solutions found online would have worked before, as of Chromium v51, event listeners are now [passive by default](https://developers.google.com/web/updates/2016/06/passive-event-listeners), which means they don't automatically block behavior.
...
As I started focusing on kiosk-related solutions, such as [this post](https://stackoverflow.com/questions/22999829/disable-chrome-pinch-zoom-for-use-in-kiosk), it seemed like there might be a better way to just disable the pinch-zoom gesture at the page level, while leaving it intact for components, such as a map, that might need it. It turns out there are **many** less-than-perfectly documented Chromium flags, which can be found on [this list that is automatically generated from the Chromium codebase](https://peter.sh/experiments/chromium-command-line-switches/).

Would a simple startup flag work? Let's find out.

## Not Chromium but CEF

It looked like I found a solution! Just pass `--disable-pinch` as a startup flag, and all will work magically. But here's the twist: we don't use Chromium itself; we use a proprietary player that uses [Chromium Embedded Framework](https://en.wikipedia.org/wiki/Chromium_Embedded_Framework) (CEF).

Now, based on some [cursory reading](https://bitbucket.org/chromiumembedded/cef/issues/1059/cef3-multi-touch-support-with-offscreen), it looked like CEF might support command-line flags, but the player we use would have to support them as well. Sadly, the player does not.
...
```
[@viewport](http://twitter.com/viewport) {
  width: device-width;
  zoom: 1.0;
  min-zoom: 1.0;
  max-zoom: 1.0;
...
Some background: in general, when you're using [X Window System](https://www.x.org) on Linux, input goes through [xinput](https://www.x.org/archive/X11R7.7/doc/man/man1/xinput.1.xhtml) before going to your apps. This is how it works for pointing devices, like mice. A word of caution as well: while this section seems brief, it involved looking at both Chromium and kernel source code, which while awesome from a learning perspective, they are both time consuming!
...
It turns out there's a CSS property called `touch-action` that lets a web developer specify the panning/pinch-zoom behavior in their web app. You can check out the docs [here on MDN](https://developer.mozilla.org/en-US/docs/Web/CSS/touch-action).
...
Good news: this is fixed in Chromium v72 ([check out this bug report and read about why Google Maps was one of the reasons it wasn't fixed for a long time](https://bugs.chromium.org/p/chromium/issues/detail?id=771330)).
...
Interested in solving smart city challenges? [We're always hiring](https://rebrand.ly/ixnjobsmedium)!